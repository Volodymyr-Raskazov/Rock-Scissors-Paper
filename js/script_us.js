...
function resultRound() {
	...
	if (opponentSelections == 1) {
		opponentSelections = "Rock";
		opponentBlock.innerText = opponentSelections;
	} else if (opponentSelections == 2) {
		opponentSelections = "Scissors";
		opponentBlock.innerText = opponentSelections;
	} else {
		opponentSelections = "Paper";
		opponentBlock.innerText = opponentSelections;
	};
	/**
	 * Камінь = 1
	...
	if (playerSelection == opponentSelections) {
		...
		resultBlock.innerText = "Draw";
	} else if (
		(playerSelection == "Rock" && opponentSelections == "Ножиці") ||
		(playerSelection == "Scissors" && opponentSelections == "Папір") ||
		(playerSelection == "Paper" && opponentSelections == "Камінь")
	) {
		timerBlock.style.display = "none"; // прибираємо таймер з екрану
		resultBlock.style.display = "inline-block";
		...
		resultBlock.innerText = "You lost";
		plusPointOpponent();
	};
	stopRound();
}
